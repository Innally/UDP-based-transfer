...
void getfile(int fd)
{
    ...
    {       
        ...
        for(int i=0;i<5;i++)
        {
            ...
            //unsigned short checkhere= temp.cal_checksum();
            if(temp.order>order+5)
                continue;
                //throw the pack
            else if(temp.checksum ==12)
                {
                    cout<<temp.checksum<<endl;
                    continue;//throw the pack
                }
              //  continue;
            else
            {
            ...
            if (temp.sendsize<512)//end of the file;
                {
                    eofflag=true;
                    break;
                }
...
int acknowledge(int fd,int order)
{
    //cout<<order<<endl;
    char buf[4] ;
    memset(buf,0,4);
